...
def show_about_page():
    ...
    with st.expander('Show', expanded=True):
        ...
        with col1:
            st.image('images/shap_feature_importance_1.png', width=600)
        with col2:
            st.image('images/shap_feature_importance_2.png', width=600)
        st.write('The two above feature importance plots are generated on the SHAP values of the trained XGBClassifier model. They represent the same feature importances using dots and bars, respectively.' + \
                'In the left plot, each dot represents a SHAP value. Red dots represent positive SHAP values (the feature increases the probability of heart disease) and blue dots represent negative SHAP values (the feature decreases the probability of heart disease). ' + \
                'Note that the order of the features is different from XGBClassifier\'s feature importance plot. This is because SHAP values and XGBClassifier\'s feature importance (weight) are fundamentally different.' + \
                'But the two plots are still consistent with each other. Heart attack, age, and general health are still the top 3 important features.')
    ...
    with st.expander('Show', expanded=True):
        ...
        st.write('2. **Data cleaning:** There are still a lot of missing values in the dataset. A lot of participants simply didn\'t answer some questions. If I drop all ' + \
                    'the rows with missing values, I will lose a lot of data (more than 50%). I didn\'t impute the missing values because I don\'t want to introduce bias as ' + \
                    'the XGBClassifier model can handle missing values.')
        st.write('3. **Class Imbalance:** The biggest challenge of this project is dealing with the imbalanced dataset. 94.4% of the participants don\'t have heart disease. If I just predict that ' + \
                    ...
                    'because we are not losing any data (as in random undersampling) and we are not introducing bias (as in random oversampling and SMOTE). The AUC is used as the evaluation metric.')
    ...
    with st.expander('Show', expanded=True):
        ...
        st.write('3. **Develop more ways to mitigate class imbalance:** The models I tested are XGBClassifier, LogisticRegression, SVC, and Simple Feedforward Neural Network.' + \
                    'My intuition is that experimenting with different models will not increase the F1 score much because most models are not designed to handle class imbalance. ' + \
                    'I would like to experiment with different ways to mitigate class imbalance, such as algorithmic methods.')
